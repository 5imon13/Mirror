...
def cut_pic(path, filenames):
    print('path:'+path)
    for img in filenames:
        print('img:'+os.path.split(img)[1])
    filenames.sort()
    ...
    for index_img, img in enumerate(images):
        ...
        if img is None:
            print('Image not found...:'+str(index_img))
            continue
        ...
        if detections[0] is not None:
            ...
            for x1, y1, x2, y2, conf, cls_conf, cls_pred in detections:
                ...
                try:
                    os.listdir(cut_path+'part_cut/upper')
                    os.listdir(cut_path+'main_cut')
                except FileNotFoundError:
                    os.makedirs(cut_path+'part_cut/upper')
                    os.makedirs(cut_path+'part_cut/down')
                    os.makedirs(cut_path+'part_cut/entire')
                    os.makedirs(cut_path+'main_cut')
                ...
                if int(cls_pred) in range(0, 6):
                    ...
                    cv2.imwrite(cut_path+'part_cut/upper/%s_%d_%d.jpg'%(classes[int(cls_pred)], index_img, cls_pred), img_cut)
                    ...
                    x_part.extend([abs(int(x1)), abs(int(x2))])
                    y_part.extend([abs(int(y1)), abs(int(y2))])
                    cv2.imwrite(cut_path+'part_cut/down/%s_%d_%d.jpg'%(classes[int(cls_pred)], index_img, cls_pred), img_cut)
                    ...
                    x_part.extend([abs(int(x1)), abs(int(x2))])
                    y_part.extend([abs(int(y1)), abs(int(y2))])
                    cv2.imwrite(cut_path+'part_cut/entire/%s_%d_%d.jpg'%(classes[int(cls_pred)], index_img, cls_pred), img_cut)
            img_sol = img[min(y_part):max(y_part), min(x_part):max(x_part)].copy()
            cv2.imwrite(cut_path+'main_cut/%d.jpg'%index_img, img_sol)
...
if __name__ == '__main__':
    ...
    path = "crawler_pics/"
    alldir, filenames = getallfiles(path)
    #if filenames:
    #    cut_pic(path, filenames)
    for dirname in alldir:
        print(2)
        #tempfilenames = getallfiles_rec(path)
        print(path+dirname+'/'+"*/*.jpg")
        tempfilenames = glob.glob(path+dirname+'/'+"*.jpg")
        if tempfilenames:
            cut_pic(path+dirname+'/', tempfilenames)
        tempfilenames = glob.glob(path+dirname+'/'+"*/*.jpg")
        if tempfilenames:
            cut_pic(path+dirname+'/', tempfilenames)
